...
from src.DataScience_Algorithms.pipeline.LogisticRegression_Pipeline import LogisticRegression_Execution
...
class LogisticRegression_RunCommand:
    ...
    def LogisticRegression_PipelineInitiate(self):
        print("main : ", self.x, self.y, self.ValueToPredict, self.weights, self.bias, self.learning_rate, self.epochs)
        # You probably want to call a LogisticRegression_Execution instead of LinearRegression_Execution
        logisticModelResult = LogisticRegression_Execution(  # <-- FIX this if needed
